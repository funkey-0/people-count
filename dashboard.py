...
def read_visitor_log():
    ...
    with open(LOG_FILE, newline="", encoding="utf-8") as f:
        for row in csv.DictReader(f):
            try:
                times.append(datetime.strptime(row["thoi_gian"], "%Y-%m-%d %H:%M:%S"))
...
def main():
    ...
    fig.suptitle("THONG KE KHACH - People Counting System", fontsize=14, fontweight="bold")
    ...
    if any(values):
        ...
        ax1.text(peak, max(values), f" cao diem {peak}h", color="red", va="top")
    ...
    if days:
        ...
        ax2.text(0.5, 0.5, "Chua co daily_counts.csv", ha="center", va="center")
        ax2.axis("off")
